...
public class AisinoPlatformShoplogsAsyncHbaseEventSerializer implements AsyncHbaseEventSerializer {
  private byte[] table;
  private byte[] cf;
      ...
      try {
        ...
        switch (keyType) {
          ...
          case CUSTOMHASH:
        	  ...
        	  System.out.println("payload[0]---------------------------------->"+Arrays.toString(payload[0]));
        	  rowKey = SimpleRowKeyGenerator.getCustomHashKey(rowSuffix,payloadvalue);              
              System.out.println("<=====================CUSTOMHASH:rowKey=====================>"+rowKey);
              break;
          default:
    ...
    if(pCol != null && !pCol.isEmpty()) {
      if(suffix.equals("timestamp")){
        ...
        System.out.println("--------------->COME IN UUID");
     	...
     	String[] pCols = pCol.replace(" ", "").split(",");//userid#date#module#collogs
  ...
  public void setEvent(Event event) {
	...
	if (subBody.length == this.payloadColumn.length)
	{
		...
		for (int i = 0; i < subBody.length; i++)
		{
			...
			System.out.println("payload[i]--------->"+Arrays.toString(payload[i]));
			if ((new String(this.payloadColumn[i]).equals(this.rowSuffixCol)))
			{
				...
				System.out.println("------------------------------------>>>>>>>>>>>>>rowSuffix:"+rowSuffix);
			}
			else{
				this.rowSuffix = "";
			}
